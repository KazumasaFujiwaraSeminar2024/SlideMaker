...
また、grepコマンドを用いてマーカーを検索してそのマーカーの範囲内のものを抽出してスライドを生成してください。
指定したファイルから定理・定義・目的に関する明示的な記述（theorem, definition, proposition, lemma環境など）が検出されない場合、ソース内の特定環境の構文定義や独自マクロの調査してください。
そして、各スライド種別について明示されていない場合は省略してください
最後に、環境未定義: \begin{theorem}や\begin{definition}といった明示的環境は見つからず、論理構造は自然文および見出しに埋め込まれている場合があります。
その際は、自然文や見出しに埋め込まれているものを対象とします。
...
生成するスライドは文字のみだと聞き手が聞いていて退屈になってしまうので、視覚的にも説明できるようにファイル内の数式は要所に説明と共に出力
