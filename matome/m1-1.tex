...
\setmainfont{TeX Gyre Termes} % 例: 欧文フォントを設定
\setsansfont{TeX Gyre Heros} % 例: サンセリフフォントを設定
\setmonofont{TeX Gyre Cursor} % 例: モノスペースフォントを設定

\setmathfont{TeX Gyre Termes Math} % 数学フォントを設定

% 和文フォント設定
\setmainjfont{Harano Aji Mincho} % 例: 和文明朝体を設定
\setsansjfont{Harano Aji Gothic} % 例: 和文ゴシック体を設定

% パッケージrgtはlualatexでは不要かもしれない、または代替が必要
% \usepackage{rgt}  % 必要に応じて対応するパッケージに変更または削除
%]]]

\begin{document}
\newcommand{\Nendo}{2024年度} % 常に「2024年度」と出力するコマンド
